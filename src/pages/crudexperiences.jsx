...
export function Crudexperiences() {
  ...
  const handleEdit = (id) => {
    route(`/updateexp/${id}`); // Mengarahkan ke /updateexp dengan ID
  };
              ...
              <tr key={exp.id} class="transition-colors duration-300">
                ...
                <td class="px-3 py-3 text-sm">
                  <div className="flex gap-2">
                    <div onClick={() => handleEdit(exp.id)} className="bg-teal-900 rounded-md p-1 cursor-pointer">
                      <Edit size={20} className="text-teal-200" />
                    </div>
                    <div onClick={() => handleDelete(exp.id)} className="bg-teal-900 rounded-md p-1 cursor-pointer">
